    ...
    for j in range(nRegions):
        ...
        for year in years:
            ...
            for month in range(1, 2):
                ...
                t[ktime] = ds.daysSinceStartOfSim.isel(Time=0).values/365.
                if 'normalTransportVelocity' in ds.keys():
                    vel = ds.normalTransportVelocity.isel(Time=0)[openBryEdges, :]
                elif 'normalVelocity' in ds.keys():
                    vel = ds.normalVelocity.isel(Time=0)[openBryEdges, :]
                    if 'normalGMBolusVelocity' in ds.keys():
                        vel = vel + ds.normalGMBolusVelocity.isel(Time=0)[openBryEdges, :]
                    if 'normalMLEvelocity' in ds.keys():
                        vel = vel + ds.normalMLEvelocity.isel(Time=0)[openBryEdges, :]
                else:
                    raise KeyError('no appropriate normalVelocity variable found')
                dzOnCells = ds.layerThickness.isel(Time=0)
                ...
                ds = ds[varlist].isel(Time=0).where(cellMask, drop=True)
