...
for month in months:
    modelfile = f'{modeldir}/{runname}.mpaso.hist.am.{mpasFile}.{year:04d}-{month:02d}-{mpasFileDayformat}.nc'
    #modelfile = f'{modeldir}/mpaso.hist.am.timeSeriesStatsMonthly.{year:04d}-{month:02d}-{mpasFileDayformat}.nc' # old (v1) filename format
    ...
    for var in variables:
        ...
        if var['is3d']:
            for iz in range(len(dlevels)):
                ...
                # dsIn  = xr.open_dataset(modelfile).isel(Time=timeIndex, nVertLevels=zlevels[iz]) # this would go outside the var loop, right after modelfile is defined
                # fld = factor * dsIn[mpasvarname].values # this would go here in place of the next 5 lines
                fld = f.variables[mpasvarname][timeIndex, :, zlevels[iz]]
                fld = ma.masked_greater(fld, 1e15)
                fld = ma.masked_less(fld, -1e15)
                fld = factor*fld
                fld = np.squeeze(fld)
                if varname=='GMkappa':
                    kappa = np.squeeze(f.variables['timeMonthly_avg_gmBolusKappa'][timeIndex, :])
                    kappa_horScaling = np.squeeze(f.variables['timeMonthly_avg_gmHorizontalTaper'][timeIndex, :])
                    fld = kappa * fld * kappa_horScaling
                print('varname=', varname, 'month=', month, 'zlev=', int(dlevels[iz]), 'fldmin=', np.min(fld), 'fldmax=', np.max(fld))
                #fld3d = f.variables[varname][timeIndex, :, :]
            ...
            if varname=='Redikappa':
                kappa_horScaling = np.squeeze(f.variables['timeMonthly_avg_RediHorizontalTaper'][timeIndex, :])
